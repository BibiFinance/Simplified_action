...
/**
 * Explication pédagogique du score (affichage Premium).
 */
function getScoreExplanation({ score_simplifie, rendement, risque }) {
  const score = score_simplifie != null ? score_simplifie : '—';
  const r = rendement != null ? rendement : 0;
  const riskPct = risque != null ? Math.round(risque * 100) : 50;
  return (
    `Note ${score}/10 : base neutre à 5, ajustée selon la variation du jour (${r} % en valeur absolue) ` +
    `et un malus lié à la volatilité intraday (indicateur de risque ${riskPct} %). ` +
    `Algorithme v${VERSION_ALGO}.`
  );
}
