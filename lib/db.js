...
// --- Actions & notations (historique des scores) ---
let actionsTableDone = false;
async function ensureActionsTable() {
  const p = getPool();
  if (!p || actionsTableDone) return;
  await p.query(`
  ...
  `);
  actionsTableDone = true;
}

let notationsTableDone = false;
async function ensureNotationsTable() {
  const p = getPool();
  if (!p || notationsTableDone) return;
  ...
  `);
  notationsTableDone = true;
}

/**
 * Met à jour l'action (ticker) et ajoute une ligne dans l'historique notations.
 */
async function saveSearchScore({ ticker, entreprise, secteur, score_simplifie, rendement, risque, version_algo }) {
  if (!isConfigured() || !ticker) return null;
  await ensureNotationsTable();
  const t = String(ticker).trim().toUpperCase();
  const actionRes = await query(
    `INSERT INTO actions (ticker, entreprise, secteur, rendement, risque)
     VALUES ($1, $2, $3, $4, $5)
     ...
     RETURNING id`,
    [t, entreprise || t, secteur || null, rendement ?? null, risque ?? null]
  );
  if (!actionRes || actionRes.rows.length === 0) return null;
  const actionId = actionRes.rows[0].id;
  await query(
    `INSERT INTO notations (action_id, note_globale, version_algo) VALUES ($1, $2, $3)`,
    [actionId, score_simplifie, version_algo || '1.0']
  );
  return actionId;
}

async function getNotationHistoryByTicker(ticker, limit = 20) {
  if (!isConfigured() || !ticker) return [];
  await ensureNotationsTable();
  const res = await query(
    `SELECT n.note_globale, n.version_algo, n.date_calcul
     FROM notations n
     JOIN actions a ON a.id = n.action_id
     WHERE UPPER(a.ticker) = UPPER($1)
     ORDER BY n.date_calcul DESC
     LIMIT $2`,
    [String(ticker).trim(), limit]
  );
  return (res?.rows || []).map((r) => ({
    note_globale: r.note_globale,
    version_algo: r.version_algo,
...
/**
 * Favoris enrichis avec dernière notation connue (dashboard).
 */
async function getFavoritesDashboard(userId) {
  const p = getPool();
  if (!p) {
    const list = (favoritesMemory.get(userId) || []).slice();
    return list.map((f) => ({
      ticker: f.ticker,
      name: f.name || f.ticker,
      score_simplifie: null,
      date_calcul: null,
      secteur: null,
    }));
  }
  ...
  await ensureNotationsTable();
  const res = await p.query(
    `SELECT f.ticker, f.name, a.secteur,
            n.note_globale AS score_simplifie,
            n.date_calcul
     FROM favorites f
     LEFT JOIN actions a ON UPPER(TRIM(a.ticker)) = UPPER(TRIM(f.ticker))
     LEFT JOIN LATERAL (
       SELECT note_globale, date_calcul
       FROM notations n2
       WHERE n2.action_id = a.id
       ORDER BY n2.date_calcul DESC
       LIMIT 1
     ) n ON TRUE
     WHERE f.user_id = $1
  ...
  return (res.rows || []).map((r) => ({
    ...
    score_simplifie: r.score_simplifie != null ? Number(r.score_simplifie) : null,
    date_calcul: r.date_calcul || null,
  }));
}
...
module.exports = {
  ...
  ensureActionsTable,
  ensureNotationsTable,
  saveSearchScore,
  getNotationHistoryByTicker,
  ensureSp500Table,
  getSp500List,
  refreshSp500,
